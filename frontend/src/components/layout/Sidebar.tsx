...
export default function Sidebar({ isOpen = true, onClose }: SidebarProps) {
  ...
  const navItems = [
    ...
    { path: "/Myproperty", icon: Building2, label: "My Properties", role: "HOST" },
    { path: "/CreateProperty", icon: PlusCircle, label: "List Property", role: "HOST" },
  ];
            ...
            <li key={item.path}>
              <Link
                ...
                style={{
                  ...
                  color: isActive(item.path) ? "var(--primary-600)" : "var(--gray-600)",
                  background: isActive(item.path) ? "var(--primary-50)" : "transparent",
                  fontWeight: isActive(item.path) ? "var(--font-medium)" : "var(--font-normal)",
